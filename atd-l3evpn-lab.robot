...
Connect To Switches
    ...
    FOR    ${veos_list}     IN      @{NODES.veos} 
        ${LEAF1}=   Set Variable If   '${veos_list.hostname}'=="leaf1"     ${veos_list.internal_ip} 
        Exit For Loop If    "${LEAF1}" != 'None'
    END
    Log    ${LEAF1}

    FOR    ${veos_list}     IN      @{NODES.veos} 
        ${LEAF3}=   Set Variable If   '${veos_list.hostname}'=="leaf3"     ${veos_list.internal_ip} 
        Exit For Loop If    "${LEAF3}" != 'None'
    END
    Log    ${LEAF3}

    FOR    ${veos_list}     IN      @{NODES.veos} 
        ${HOST1}=   Set Variable If   '${veos_list.hostname}'=="host1"     ${veos_list.internal_ip} 
        Exit For Loop If    "${HOST1}" != 'None'
    END
    Log    ${HOST1}

    FOR    ${veos_list}     IN      @{NODES.veos} 
        ${SPINE1}=   Set Variable If   '${veos_list.hostname}'=="spine1"     ${veos_list.internal_ip} 
        Exit For Loop If    "${SPINE1}" != 'None'
    END
    Log    ${SPINE1}

    FOR    ${veos_list}     IN      @{NODES.veos} 
        ${SPINE2}=   Set Variable If   '${veos_list.hostname}'=="spine2"     ${veos_list.internal_ip} 
